...
- (instancetype)initWithPlist: (NSString *)name {
    ...
    if (self) {
        ...
        if (path == nil) {
            path = [[NSBundle bundleWithPath:yfDemoBundlePath] pathForResource:name ofType:@"plist"];
        }
...
- (NSDictionary *)getFieldsData {
    ...
    for (NSDictionary *dic in self.textFields) {
        ...
        for (YFTextField *field in fields) {
...
- (YFTextField *)fieldAtIndexPath: (NSIndexPath *)indexPath {
    ...
    YFTextField *field = [fields objectAtIndex:indexPath.row];
    return field;
}
...
- (void)parseFields {
    ...
    for (NSDictionary *dic in self.textFields) {
        ...
        for (NSString *key in fieldsArr) {
            YFTextField *field = [[YFTextField alloc] initWithKey:key andTarget:self.target];

...
@implementation YFTextFieldModel
...
- (NSString *)cachePath {
    NSString *path = [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"] stringByAppendingPathComponent:@"InputFields.plist"];
    return path;
}
...
@interface YFTextField () <UIPickerViewDelegate, UIPickerViewDataSource>
...
- (void)parseField {
    YFTextFieldModel *model = [[YFTextFieldModel alloc] initWithKey:self.key];
    self.model = model;
    [self setFieldWithModel:model];
}

- (void)setFieldWithModel: (YFTextFieldModel *)model {
    self.frame = CGRectMake(18, 0, kWindowW - 26, 44);
...
- (void)configRightView {
    ...
    [self.rightBtn setTitleColor:[YFColor isEqual:[UIColor whiteColor]]?kYFNavTextColor:YFColor forState:UIControlStateNormal];
    if (self.model.rightViewAction.length > 0 && self.target) {
        if ([self.target respondsToSelector:NSSelectorFromString(self.model.rightViewAction)]) {
            [self.rightBtn addTarget:self.target action:NSSelectorFromString(self.model.rightViewAction) forControlEvents:UIControlEventTouchUpInside];
        }
    }
    self.rightView = self.rightBtn;
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        ...
        _pickerView.backgroundColor = YFHexColor(0xf6f6f6);
    }
    return _pickerView;
}
...
- (UIDatePicker *)datePicker {
    if (_datePicker == nil) {
        ...
        _datePicker.backgroundColor = YFHexColor(0xf6f6f6);
